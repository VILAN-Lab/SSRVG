...
class RandomHorizontalFlip(object):
    def __call__(self, input_dict):
        if random.random() < 0.5:
            ...
            r1_word = r1_word.replace('right', '*&^special^&*').replace('left', 'right').replace('*&^special^&*',
                                                                                                 'left')
            h, w = img.height, img.width
            ...
            if 'sig_img' in input_dict:
                sig_img = input_dict['sig_img']
                sig_img = F.hflip(sig_img)
                input_dict['sig_img'] = sig_img
